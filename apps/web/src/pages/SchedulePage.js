...
export function SchedulePage() {
    ...
    const scheduleAssignees = useMemo(() => (users.data ?? []).filter((u) => Boolean(u.is_active) &&
        (u.role === "admin" || u.role === "streamer" || u.role === "shipper" || u.role === "bagger")), [users.data]);
    ...
    const editingSlot = useMemo(() => (schedules.data ?? []).find((s) => s.id === editingId) ?? null, [schedules.data, editingId]);
    const modalLaborMode = Boolean(editingSlot ? editingSlot.entry_type === "labor" : assignee && (assignee.role === "shipper" || assignee.role === "bagger"));
        ...
        mutationFn: () => {
            ...
            if (isAdmin) {
                if (editingSlot?.entry_type === "labor") {
                    ...
                    if (!Number.isFinite(h) || h <= 0)
                        throw new Error("Hours worked must be a positive number");
                    return api(`/v1/admin/schedules/${editingId}`, {
                        method: "PATCH",
                        body: JSON.stringify({ date: formDate, streamerId: formStreamer, hoursWorked: h })
                    });
                }
            ...
            return api(`/v1/schedules/mine/${editingId}`, {
                method: "PATCH",
                body: JSON.stringify({ date: formDate, startTime: formTime })
            });
        },
    ...
    const openEdit = (slot) => {
        ...
        setFormHours(slot.entry_type === "labor" && slot.hours_worked != null ? String(slot.hours_worked) : "8");
        setModalOpen(true);
    };
    const onSubmit = (e) => {
        e.preventDefault();
        if (!formDate || (isAdmin && !formStreamer))
            return;
                    ...
                    return (_jsxs("div", { style: {
                            ...
                            overflow: "hidden"
                                    ...
                                    borderBottom: "1px solid var(--border)"
                                            ...
                                            borderRadius: 2
                                        }, children: "Nothing scheduled" })) : (slots.map((s) => {
                                ...
                                : isAdmin
                                    ...
                                    : "Request scheduled stream" }), _jsxs("form", { onSubmit: onSubmit, children: [isAdmin ? (_jsxs("div", { className: "form-group", children: [_jsx("label", { className: "form-label", htmlFor: "sc-streamer", children: "User" }), _jsx("select", { id: "sc-streamer", className: "form-input", value: formStreamer, onChange: (e) => setFormStreamer(e.target.value), children: scheduleAssignees.map((u) => (_jsxs("option", { value: u.id, children: [userLabel(u), " (", u.role, ")"] }, u.id))) })] })) : null, _jsxs("div", { className: "form-group", children: [_jsx("label", { className: "form-label", htmlFor: "sc-date", children: "Date" }), _jsx("input", { id: "sc-date", className: "form-input", type: "date", value: formDate, onChange: (e) => setFormDate(e.target.value) })] }), modalLaborMode ? (_jsxs("div", { className: "form-group", children: [_jsx("label", { className: "form-label", htmlFor: "sc-hours", children: "Hours worked" }), _jsx("input", { id: "sc-hours", className: "form-input", type: "number", min: 0.01, step: 0.25, value: formHours, onChange: (e) => setFormHours(e.target.value) })] })) : (_jsxs("div", { className: "form-group", children: [_jsx("label", { className: "form-label", htmlFor: "sc-time", children: "Start time" }), _jsx("input", { id: "sc-time", className: "form-input", type: "time", value: formTime, onChange: (e) => setFormTime(e.target.value) })] })), mutError ? _jsx("p", { className: "error", children: mutError.message }) : null, reviewMut.error ? _jsx("p", { className: "error", children: reviewMut.error.message }) : null, _jsxs("div", { className: "modal-actions", children: [_jsx("button", { type: "button", className: "btn btn-outline", onClick: closeModal, children: "Cancel" }), _jsx("button", { type: "submit", className: "btn btn-gold", disabled: pending, children: isAdmin ? "Save" : "Submit" })] })] })] }) })] }));
}
